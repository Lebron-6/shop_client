...
export default {
  ...
  mounted() {
    ...
    // https://www.swiper.com.cn/usage/swiper4/index.html
    // 创建一个swiper实例对象,来实现轮播
    // 问题:创建时机 需要在数组数据来了之后再去创建
  },
  computed: {
    ...mapState(["address", "categorys", "userInfo"]),
    // 需求:需要把getCategorys数据处理成二维数组
    // 小数组中的元素个数最大是8个
    categorysArr() {
      ...
      categorys.forEach((c) => {
        if (minArr.length === 8) {
          // 如果当前小数组已经满了  再创建新的小数组
          minArr = [];
        }
        if (minArr.length === 0) {
          //如果minArr是空的(即新创建的小数组),将小数组保存到大数组中
          arr.push(minArr);
        }
        minArr.push(c);
        // 将当前分类保存到小数组中
  ...
  watch: {
    categorys(value) {
      // categorys数组中有数据了,再异步更新界面之前执行

      // 1.可以使用setTimeout实现效果,但是不推荐使用
